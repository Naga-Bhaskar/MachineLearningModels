...
class MyLogisticRegression:
  ...
  def logLikelihood(self, X, y):
    ...
    probability1 = np.sum(y*np.log(h_theta))
    probability0 = np.sum((1-y)*np.log(1-h_theta))
    ...
    for iter in range(epoch):
      ...
      for i in range(0, m - batch_size + 1, batch_size):
        ...
        for j in np.arange(n):
          offsets = self.logistic(X_batch.dot(self.theta)) - y_batch
          self.theta[j] = self.theta[j] - alpha * (1 / batch_size) * offsets.T.dot(X_batch[:, j])
...
def test(X_train, y_train, X_test, y_test, alpha, epoch):
...
def main():
  ...
  test(X_train, y_train, X_test, y_test, 0.05, 100)
